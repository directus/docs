...
| `ACCEPT_TERMS`                             | Confirm acknowledgement of the [Monospace Sustainable Core License](https://directus.io/MSCL).                       | `false`                      |
...
When `LICENSE_KEY` or `LICENSE_TOKEN` is set in the environment, the Studio license editor is read-only. To change the license, update the environment variable and restart Directus.
::
