...
| `IMPORT_MAX_FILE_SIZE`        | The maximum size of a file uploaded to the import endpoints, or of a schema snapshot uploaded to `/schema/diff` and `/schema/apply`. Accepts number of bytes, or human readable string. Returns `413` when exceeded. | `50mb`        |
