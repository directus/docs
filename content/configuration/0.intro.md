...
Environment vairables that end on a `*` in our docs allow you to configure internal objects with the following notation.

For the variable `MY_ENV_*`:
- `MY_ENV_SIZE=1` will be turned into `{ size: 1 }`
- `MY_ENV_MAX_LENGTH=1` will be turned into `{ maxLength: 1 }`
- `MY_ENV_BRAND__COLOR=red` will be turned into `{ brand: {color: 'red'} }`
- `MY_ENV_BRAND__FONT_SIZE=18px` and `MY_ENV_BRAND__FONT=arial` will be turned into `{ brand: { fontSize: '18px', font: 'arial' } }`
