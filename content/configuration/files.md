...
| `ASSETS_CACHE_REVALIDATE`                | When enabled, assets use `Cache-Control: max-age=0, must-revalidate` instead of `max-age`. CDNs revalidate via `If-Modified-Since` / `ETag` on every request, ensuring replaced files are served immediately. Note: for existing assets, the initial CDN cache must be cleared before revalidation can take effect. | `false`       |
