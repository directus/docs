...
Before applying a license, `PUBLIC_URL` must be set to a valid absolute URL — for example, `https://directus.example.com`. The default value (`/`) and any relative path are rejected by the licensing service when it tries to record the [activation](#activations-and-project-bindings).

Directus boots successfully even when `PUBLIC_URL` is not a fully-qualified URL — you will only see a warning in the logs — and activation may appear to succeed during onboarding. The mismatch surfaces later in **Settings > License**, where actions such as applying or managing a license fail with:

```
[INVALID_PAYLOAD] Invalid payload. public_url must be a valid absolute URL.
```

If you encounter this error, set `PUBLIC_URL` to your instance's public address, restart Directus, and reapply the license.

`PUBLIC_URL` is also used elsewhere in Directus — for OAuth redirects, password-reset emails, and other public-facing links. See [`PUBLIC_URL` in the general configuration reference](/configuration/general) for the full description.
