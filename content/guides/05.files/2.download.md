...
Their are three primary ways to download content within the directus:
...
3. Locate the **Download** button (:icon{name="material-symbols:download"}) in the top-right of the page.
4. Click **Download** to begin downloading the file.

![File Detail Page download]()
...
4. Click the **Download** button ((:icon{name="material-symbols:download"})) in the toolbar.
5. A ZIP file containing all selected files will save to your downloads

![File Library with multiple files selected for download]()
...
![Folder Tree Sidebar with Folder Context Menu showing download folder menu item]()
...
### Minimim Required Permissions

#### Downloading Multiple Files as a ZIP

Requires a read permissions on the `directus_files` collection with access to the `id` field.

#### Downloading a Folder as a ZIP

Requires a read permissions on the `directus_folders` collection with access to the `id` field and the `directus_files` collection with access to the file `id` and `folder` fields.
