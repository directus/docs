...
When backlink is set to `false`, the API will exclude reverse relations during `*.*` wildcard expansion to prevent circular references and reduce duplicate data in responses. The backlink parameter defaults to `true`, so you need to explicitly set it to `false` to enable the filtering behavior.
