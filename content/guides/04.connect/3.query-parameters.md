...
When backlinks is set to `false`, the api will expand `*.*` to exclude all fields that where previously followed to remove duplicate entries.
...
::callout{icon="material-symbols:info-outline"}
**Example**  
Red lines mark the response when backlinks is set to true while green marks when backlinks is set to false.
