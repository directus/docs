...
The selectable resources are `roles`, `policies`, `flows`, `dashboards`, `settings`, `folders`, `users`, and `translations`. The CLI automatically includes `access`, `permissions`, `operations`, and `panels` with their parent resources (see [the dependency table](#configuration-resources)). `--flows` also brings `folders`, because the folder a flow lives in is part of its identity. Folders differ from the automatic resources in one way: `folders` is selectable in its own right, so `--no-deps` drops it from a `--flows` selection while `operations` still comes. Positive selection (`--flows`) cannot be combined with `--all` or with `--no-` flags. An exclusion that a retained resource depends on is refused rather than silently overridden: `--no-policies` alone fails because `roles` requires policies and would pull them back, so pass `--no-roles` as well, or keep policies. `--no-folders` alone fails for the same reason, because `flows` requires folders. Resource selection never narrows the schema; the two axes are scoped independently.
...
| `flows`        | Yes                          | `--flows`        | `operations`, `folders` | Folders come along because the folder a flow lives in is part of how it is matched.                                                                                                                                                                    |
...
| `folders`      | Yes                          | `--folders`      | —                       | Both folder trees. Media-library folders and flow folders share `directus_folders` and are told apart by their `type`. Distinct from collection folders (Data Studio sidebar groups), which sync as Schema.                                            |
...
| `... --flows`                        | All overwritten (resource flags do not narrow Schema) | Flows, operations, and folders overwritten; all others left unchanged   |
| `... --flows --no-schema`            | All left unchanged                                    | Flows, operations, and folders overwritten; all others left unchanged   |
...
| Resource                                        | Matched by                           |
| ----------------------------------------------- | ------------------------------------ |
| Roles, policies, and dashboards                 | `name`                               |
| Flows                                           | `name` and `folder`                  |
| Folders                                         | `name`, `parent`, and `type`         |
| Users                                           | `email`                              |
