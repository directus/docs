...
The `_some` and `_none` filter operators are used for filtering One-to-Many and Many-to-Many relational fields in API queries. These allow you to filter items based on conditions applied to their related collections.

The `_some` operator retrieves records where at least one related item meets the condition.
By default, Directus will apply the `_some` operator when querying One-to-Many and Many-to-Many relational queries.

::example

This filter retrieves all items where at least one related category has the name "Recipe":
...
In the above, `categories` is the relational field in the collection. `_some` checks if at least one related category has the name "Recipe". If an article has one or more categories named "Recipe", it will be included in the result.

::

The `_none` operator retrieves records where none of the related items meet the condition.

::example
This filter retrieve all items where none of th categories has the name "Recipe":
...
```
In the above, `categories` is the relational field in the collection. `_none` checks if no related category has the name "Recipe". If an article has no categories named "Recipe", it will be included in the result.
::
