...
Using `$FOLLOW(target-collection, relation-field)` you can filter items by traversing relations in reverse. This is useful when there is a relation from the target collection to the current collection, but no relation field has been configured in the current collection to the target collection.

::example
There is an M2O relation from books to category but a category does not have a `books` field as it is also used by other collections like movies and it would be messy to a O2M field on the categories collection for each collection that has a M2O relation to it. In this case, you can use the `$FOLLOW` syntax to filter categories by books that have a reference to them.

```json
{
  "$FOLLOW(books, category)": {
    "name": {
      "_eq": "Monthy Python and the Holy Grail"
    }
  }
}
```
