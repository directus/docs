...
You can specify related values by nesting field names.

::example

If you have a relational Many to One `author` field, and want to specify a filter on the author's `name` field:
```json
{
  "authors": {
    "authors_id": {
...
When applying filters to a One to Many field, Directus will default to a 'some' search, meaning it will return all values that match.
...
::example
To specify that none of the categories have the name of 'Recipe':
```json
{
  "categories": {
    "_none": {
