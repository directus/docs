...
- **Iteration Mode** - Specifies how triggered flows are executed. Supported modes are `Serial`, `Parallel`, and `Batch` (default: `Serial`).
  ...
  - `Batch`: Runs the selected flow in batches based on the configured `Batch Size`. All items in the payload are grouped into a batch and each item in the batch is executed in parallel. The batches themselves are processed sequentially, with each starting only after the previous completes.
