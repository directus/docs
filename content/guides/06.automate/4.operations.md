...
- **Iteration Mode** - Specifies how triggered flows are executed. Supported modes are `Serial`, `Parallel`, and `Batch` (default: `Serial`).
  ...
  - `Batch`: Runs the selected flow in batches based on the configured `Batch Size`. Each item in the payload is grouped into a batch and executed in parallel. The batches themselves are processed sequentially, with each starting only after the previous completes.
