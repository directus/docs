...
- **Iteration Mode** - Specifies how triggered flows are executed. Supported modes are `Serial`, `Parallel`, and `Batch` (default: `Serial`).
  - `Serial`: Triggers the selected flow sequentially, starting the next only after the previous execution completes.
  - `Batch`: Triggers the selected flow in batches based on the configured `Batch Size`. Flows within each batch executre in parallel, while batches are processed sequentially, with each starting only after the previous completes.
  - `Parallel`: Triggers the selected flow for all items concurrently and waits for all executions to complete, with no guaranteed order.
- **Batch Size** - Available when `Iteration Mode` is set to `Batch`; defines the number of items processed per batch (default: 10).
