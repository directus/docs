...
The password policy (Settings → Auth → Password Policy) is a regular expression evaluated against every submitted password. The built-in **Weak** and **Strong** presets are safe, but a custom pattern with nested quantifiers (`(a+)+$`) can be vulnerable to catastrophic backtracking. Because the match runs on the main event loop and is evaluated wherever passwords are set, including public registration and password reset, a slow pattern can be triggered remotely to block the instance.
