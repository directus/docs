...
- A **Manual** trigger flow lists its target collections in the `options` field, potentially exposing the names of collections a user has no permission to access. To prevent this keep flows used by lower-privileged users separate from those that reference restricted collections.
...
Read access matters too. `directus_settings` stores the API keys for your configured AI providers in fields such as `ai_openai_api_key`, so a policy that grants read on the whole collection hands those credentials to every user it applies to.
...
- Scope read on `directus_settings` to the fields your users actually need. Policies created before Directus 12.2.0 grant read on every field, including the AI provider keys. See [Version 12 breaking changes](/releases/breaking-changes/version-12) for the field list applied to new policies.
...
### Use Safe Password Policy Patterns

The password policy (Settings → Auth → Password Policy) is a regular expression evaluated against every submitted password. The built-in **Weak** and **Strong** presets are designed to be safe. However, custom patterns have no such restriction, adding a custom rule with nested quantifiers (e.g. `(a+)+$`) can be vulnerable to catastrophic backtracking. 

Since password validation runs on the main event loop and is performed whenever a password is created or updated (including public registration and password resets) a poorly designed pattern could be exploited remotely to degrade performance or make the instance unresponsive.

- Prefer built-in presets whenever possible, as they are safe by design.
- When using a custom regex, avoid nested quantifiers (e.g. `(x+)+`, `(x*)*` etc) and test the pattern against long, non-matching inputs before deploying it.
...
:::card{title="Security & Limits" icon="i-lucide-settings" to="/configuration/security-limits"}
...
:::card{title="Report a Security Issue" icon="i-lucide-lock-keyhole" to="/community/reporting-and-support/security-reporting"}
