...
- Different resources often require different levels of network security.
- Combine multiple policies with varying IP access allowing for enhanced local or developer access
...
when applying multiple policies to a role or user they combine in the following way:
- ip access is subtractive (removes entire policy from the chain if not applicable)
- field permissions for the same collection are additive, so will be combined into a list of fields
- item rules are additive and will be merged for the same collection