...
When allowing non admin users to update `directus_settings`, users might be able to gain information outside of their permissions. Allowing users to update the `theming_group` for example allows non admin users to inject any css into the app which can have unforseen consequences. To reduce the risk of abuse with css injections, set `CONTENT_SECURITY_POLICY__DIRECTIVES_**` to only allow trusted domains and sources.
