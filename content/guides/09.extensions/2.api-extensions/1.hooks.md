...
::callout{icon="i-lucide-book-open" color="primary" to="/guides/realtime/custom-handlers"}
...
When using the sandbox, you have access to `filter` and `action` events only. Callback functions receive the `payload` object as the only parameter.
