...
_(rather than being explicitly `null`)_, the condition takes the reject path regardless of the operator. To handle
...
::callout{icon="i-lucide-triangle-alert" color="warning" to="/configuration/email"}
...
- **To** — The recipient email addresses<sup>[1]</sup>.
...
<sup>[1]</sup> These interfaces are [tags](/guides/data-model/interfaces#tags).
