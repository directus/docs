...
| `--allow-drift`             | Preview despite a Directus version or database vendor mismatch (see [the compatibility rule](#the-compatibility-rule)) |
| `--project <name>`          | Project to sync (default: `default`)                                                                 |
...
| `--allow-drift`              | Push despite a Directus version or database vendor mismatch (see [the compatibility rule](#the-compatibility-rule)) |
| `--project <name>`           | Project to sync (default: `default`)                                                                      |
...
1. A `--token` flag on `profile add`, `profile update`, or `profile test`. The sync commands take no token flag.
...
3. The static token or login session in `~/.directus/credentials.json`, written readable only by you (mode `0600`). Never consulted when `CI` is non-empty, except when its value is `false`.
