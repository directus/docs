---
title: Tools
description: Reference of all tools available to AI Chat for interacting with your Directus instance.
headline: AI Chat
---

AI Chat uses tools to perform actions on your behalf. Each tool handles a specific type of operation within Directus.

::callout{icon="material-symbols:science" color="warning"}
**AI Chat is in beta.** Tools and capabilities may change.
::
...
Page Context tools let the AI work directly with what's on your screen. They are only available when editing data and may not be available on all pages.
::
| Tool | Description |
|------|-------------|
| :icon{name="material-symbols:visibility-outline" class="text-muted"} **Read Form Values** | Read current field values from the form on screen |
| :icon{name="material-symbols:edit-outline" class="text-muted"} **Set Form Values** | Update field values on the current form |
...
![Tool configuration panel](https://placehold.co/500x400/1E1E32/FFFFFF?text=Tool+Configuration+Panel)
