...
Use `MCP_OAUTH_DCR_ENABLED=true` for [Dynamic Client Registration](#dynamic-client-registration), `MCP_OAUTH_CIMD_ENABLED=true` for [Client ID Metadata Document registration](#client-id-metadata-document), or both if you intentionally support both onboarding paths.
...
MCP OAuth support depends on the MCP client, not the model provider. Record a client here only after testing it against Directus with a specific client version and date.

| Client | Tested | Registration Method | Enable | Notes |
| ------ | ------ | ------------------- | ------ | ----- |
| _No clients documented yet._ | - | - | - | Add a row after verifying OAuth discovery, browser authorization, token exchange, and MCP access. |
