...
MCP_OAUTH_CIMD_ALLOWED_DOMAINS=client.example.com
...
::callout{icon="material-symbols:person-add" color="primary"}
...
::callout{icon="material-symbols:person" color="warning"}
...
| Client | OAuth Registration | Setup |
| ------ | ------------------ | ----- |
| Claude Desktop | CIMD preferred. DCR also works. | Add a custom connector with the Directus MCP URL. |
| Claude.ai | CIMD preferred. DCR also works. | Add a custom connector with the Directus MCP URL. |
| Claude Code | CIMD preferred. DCR also works. | Add the Directus MCP URL with the CLI. |
| ChatGPT | DCR | Use OAuth and open **Advanced OAuth settings** during connector setup. |
| Raycast | DCR | Add the Directus MCP URL in Raycast's MCP server settings. |
| VS Code | CIMD | Add an HTTP MCP server. VS Code does not fall back to DCR. |
| Cursor | DCR | Use the install button or add the MCP server URL manually. |

#### OAuth Examples

::accordion{type="single"}

:::accordion-item{label="Claude Desktop and Claude.ai" icon="i-simple-icons-anthropic"}
...
#### Static Token Examples

Use these examples for clients or setups that do not support MCP OAuth.
