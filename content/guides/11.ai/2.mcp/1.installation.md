...
| Method | Use When |
| ------ | -------- |
| **OAuth** | Your MCP client supports remote server OAuth discovery and browser authorization. Users sign in to Directus and approve access from a consent page. |
| **Static Access Token** | Your MCP client requires a manually configured bearer token, header, or query parameter. |
