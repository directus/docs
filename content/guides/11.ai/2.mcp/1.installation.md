...
:::tabs-item{label="OAuth" icon="material-symbols:passkey"}
...
For self-hosted projects, enable the OAuth server and at least one client registration method:
...
Use `MCP_OAUTH_DCR_ENABLED=true` for [Dynamic Client Registration](/guides/ai/mcp/oauth#dynamic-client-registration), `MCP_OAUTH_CIMD_ENABLED=true` for [Client ID Metadata Document registration](/guides/ai/mcp/oauth#client-id-metadata-document), or both if you intentionally support both onboarding paths.
...
:::tabs-item{label="Static Access Token" icon="material-symbols:key"}

Static access tokens work with clients that do not support MCP OAuth. Create a dedicated Directus user for AI access, then generate a token for that user.
