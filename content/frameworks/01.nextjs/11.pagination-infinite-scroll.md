...
async function Pagination({ limit, currentPage }) {
  ...
  const hasMorePage = () => {
    const receivedPostsCount = limit * currentPage;
    return receivedPostsCount < totalPostCount;
  };
...
Infinite scrolling requires browser events or the Intersection observer JavaScript API to be implemented so it needs to be done on the client side but one way we can improve this is to load the initial HTML from the server. For that, we will be using [Server Actions](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations).
...
const PostList = ({ initialPosts, getPosts, limit, totalPostCount }) => {
  ...
  const hasMorePosts = () => {
    ...
    return receivedCount < totalPostCount;
  };
