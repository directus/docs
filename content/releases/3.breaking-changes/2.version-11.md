...
Arguments of service methods are now strictly typed which can result in type errors for broader types that were accepted before:
...
**Workaround**  
  :::collapsible{name="type workarounds"}
    When running into type errors casting the services to `any` will result in the old behavior.
