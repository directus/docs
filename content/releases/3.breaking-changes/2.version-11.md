...
We now require user authentication when calling the endpoint to trigger flows with a manual trigger.
If you need a Flow to be publicly accessible then a webhook trigger should be used instead.
