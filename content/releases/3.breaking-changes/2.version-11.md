...
#### Long running imports can now timeout
...
#### Collaborative editing types have moved

Exports previously available from `@directus/types/collab` are now exported directly from `@directus/types`

#### The app UI has been shrunk to 90% of its previous size

Extensions that rely on hardcoded px values or the old 14px root font-size may render incorrectly — all app sizing now uses rem based on the 16px browser default.

## Version 11.16.0

#### A version with a key of 'draft' now has meaning

Existing versions with the key "draft" and a custom name other than "Draft" will have its display name standardized to “Draft”.

#### Added field and version access checks for visual editing

Field permissions and version access checks require `@directus/visual-editing` v2.0.0+.

#### UsersService.requestPasswordReset throws Forbidden on external auth users

The `UsersService.requestPasswordReset` now errors for external auth provider users to prevent erroneuos emails.

## Version 11.15.0

#### Disabled interfaces are no longer interactive
...
#### Non-relation types removed from RELATIONAL_TYPES
