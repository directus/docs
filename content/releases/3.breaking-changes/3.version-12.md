...
Directus 12 introduces active license enforcement. Self-hosted instances run on the **core tier** by default. Higher limits and additional features are unlocked by configuring a license. See [Licensing](/licensing/overview) for a full explanation.
...
If you are not an existing customer and do not intend to add a license, reduce usage to within core limits before the grace period ends. Note that free commercial use may be available through the [Open Innovation Grant](http://directus.io/oig).
...
- The `#headline` slot has been deprecated.
